...
IsRetirementEligible_Normal <- function(Age, YOS, EntryYear){
  Check_Special_Tier1 = ifelse(YOS >= 25 | (Age >= 52 & YOS >= 25), TRUE, FALSE)
  Check_Special_Tier2 = ifelse(YOS >= 30 | (Age >= 65 & YOS >= 8), TRUE, FALSE)
...
IsRetirementEligible_Early <- function(Age, YOS, EntryYear){
  Check_Early_Tier1 = ifelse((Age >= 55 & YOS >= 6), TRUE, FALSE)
...
ReducedFactor <- expand_grid(Age, YOS, EntryYear) %>% 
  ...
  mutate(RetType = IsRetirementEligible_Early(Age, YOS, EntryYear),
         RF = ifelse(RetType == T, 1 - YOS*(5/12/100),
